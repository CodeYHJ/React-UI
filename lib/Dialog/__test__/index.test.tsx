...
describe('Dialog', () => {
    ...
    //     const json = renderer.create(<Dialog visible={true} />).toJSON()
    //     expect(json).toMatchSnapshot()
    // })
    it('trigger onCancel once when click on cancel button', () => {
        const onCancel = jest.fn()
        const wrapper = shallow(<Dialog visible={true} onCancel={onCancel} />);
        wrapper.find('.code-ui-dialog-cancel').simulate('click')
        expect(onCancel).toBeCalled()
    })
    it('trigger onOK once when click on OK button', () => {
        const onOk = jest.fn()
        const wrapper = shallow(<Dialog visible={true} onOk={onOk} />);
